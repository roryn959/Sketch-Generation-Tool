...
def orthogonal_initializer(scale=1.0):
  """Orthogonal initializer."""
  def _initializer(shape, dtype=tf.compat.v1.float32,
                   partition_info=None):  # pylint: disable=unused-argument
    return tf.compat.v1.constant(orthogonal(shape) * scale, dtype)
...
def lstm_ortho_initializer(scale=1.0):
  ...
  def _initializer(shape, dtype=tf.compat.v1.float32,
    ...
    return tf.compat.v1.constant(t, dtype)
...
class LSTMCell(tf.compat.v1.nn.rnn_cell.RNNCell):
  ...
  def get_output(self, state):
    unused_c, h = tf.compat.v1.split(state, 2, 1)
    return h

  def __call__(self, x, state, scope=None):
    with tf.compat.v1.variable_scope(scope or type(self).__name__):
      c, h = tf.compat.v1.split(state, 2, 1)
      ...
      concat = tf.compat.v1.concat([x, h], 1)
      w_full = tf.compat.v1.concat([w_xh, w_hh], 0)
      hidden = tf.compat.v1.matmul(concat, w_full) + bias

      i, j, f, o = tf.compat.v1.split(hidden, 4, 1)

      if self.use_recurrent_dropout:
        g = tf.compat.v1.nn.dropout(tf.compat.v1.tanh(j), self.dropout_keep_prob)
      else:
        g = tf.compat.v1.tanh(j)

      new_c = c * tf.compat.v1.sigmoid(f + self.forget_bias) + tf.compat.v1.sigmoid(i) * g
      new_h = tf.compat.v1.tanh(new_c) * tf.compat.v1.sigmoid(o)

      return new_h, tf.compat.v1.concat([new_c, new_h], 1)  # fuk tuples.
...
def layer_norm_all(h,
  ...
  h_reshape = tf.compat.v1.reshape(h, [batch_size, base, num_units])
  mean = tf.compat.v1.reduce_mean(h_reshape, [2], keepdims=True)
  var = tf.compat.v1.reduce_mean(tf.compat.v1.square(h_reshape - mean), [2], keepdims=True)
  epsilon = tf.compat.v1.constant(epsilon)
  rstd = tf.compat.v1.rsqrt(var + epsilon)
  h_reshape = (h_reshape - mean) * rstd
  # reshape back to original
  h = tf.compat.v1.reshape(h_reshape, [batch_size, base * num_units])
...
def layer_norm(x,
  ...
  axes = [1]
  mean = tf.compat.v1.reduce_mean(x, axes, keepdims=True)
  x_shifted = x - mean
  var = tf.compat.v1.reduce_mean(tf.compat.v1.square(x_shifted), axes, keepdims=True)
...
def raw_layer_norm(x, epsilon=1e-3):
  axes = [1]
  mean = tf.compat.v1.reduce_mean(x, axes, keepdims=True)
  std = tf.compat.v1.sqrt(
      tf.compat.v1.reduce_mean(tf.compat.v1.square(x - mean), axes, keepdims=True) + epsilon)
  output = (x - mean) / (std)
  return output
...
def super_linear(x,
  ...
  with tf.compat.v1.variable_scope(scope or 'linear'):
    ...
    w = tf.compat.v1.get_variable(
        'super_linear_w', [x_size, output_size], tf.compat.v1.float32, initializer=w_init)
    if use_bias:
...
class LayerNormLSTMCell(tf.compat.v1.nn.rnn_cell.RNNCell):
  ...
  def get_output(self, state):
    h, unused_c = tf.compat.v1.split(state, 2, 1)
    return h

  def __call__(self, x, state, timestep=0, scope=None):
    with tf.compat.v1.variable_scope(scope or type(self).__name__):
      h, c = tf.compat.v1.split(state, 2, 1)
      ...
      concat = tf.compat.v1.concat([x, h], 1)  # concat for speed.
      w_full = tf.compat.v1.concat([w_xh, w_hh], 0)
      concat = tf.compat.v1.matmul(concat, w_full)  #+ bias # live life without garbage.

      # i = input_gate, j = new_input, f = forget_gate, o = output_gate
      concat = layer_norm_all(concat, batch_size, 4, h_size, 'ln_all')
      i, j, f, o = tf.compat.v1.split(concat, 4, 1)
      ...
      new_c = c * tf.compat.v1.sigmoid(f + self.forget_bias) + tf.compat.v1.sigmoid(i) * g
      new_h = tf.compat.v1.tanh(layer_norm(new_c, h_size, 'ln_c')) * tf.compat.v1.sigmoid(o)

    return new_h, tf.compat.v1.concat([new_h, new_c], 1)
...
class HyperLSTMCell(tf.compat.v1.nn.rnn_cell.RNNCell):
  ...
  def get_output(self, state):
    total_h, unused_total_c = tf.compat.v1.split(state, 2, 1)
  ...
  def hyper_norm(self, layer, scope='hyper', use_bias=True):
    ...
    with tf.compat.v1.variable_scope(scope):
      ...
      alpha = super_linear(
          ...
          use_bias=False,
          scope='alpha')
      result = tf.compat.v1.multiply(alpha, layer)
      if use_bias:
        zb = super_linear(
            self.hyper_output,
            embedding_size,
  ...
  def __call__(self, x, state, timestep=0, scope=None):
    with tf.compat.v1.variable_scope(scope or type(self).__name__):
      total_h, total_c = tf.compat.v1.split(state, 2, 1)
      h = total_h[:, 0:self.num_units]
      c = total_c[:, 0:self.num_units]
      self.hyper_state = tf.compat.v1.concat(
      ...
      ih, jh, fh, oh = tf.compat.v1.split(hh, 4, 1)
      ...
      # split bias
      ib, jb, fb, ob = tf.compat.v1.split(bias, 4, 0)  # bias is to be broadcasted.
      ...
      if self.use_layer_norm:
        concat = tf.compat.v1.concat([i, j, f, o], 1)
      ...
      new_c = c * tf.compat.v1.sigmoid(f + self.forget_bias) + tf.compat.v1.sigmoid(i) * g
      new_h = tf.compat.v1.tanh(layer_norm(new_c, self.num_units, 'ln_c')) * tf.compat.v1.sigmoid(o)

      hyper_h, hyper_c = tf.compat.v1.split(hyper_new_state, 2, 1)
      new_total_h = tf.compat.v1.concat([new_h, hyper_h], 1)
      new_total_c = tf.compat.v1.concat([new_c, hyper_c], 1)
      new_total_state = tf.compat.v1.concat([new_total_h, new_total_c], 1)
